  ...
  <div
    ...
    <div
      ...
      <ul
        ...
        type="i">
        <li>
          <span class="font-semibold"> Category </span><span>:</span> {{ category }}
        </li>
        <li>
          <span class="font-semibold">Per Price </span> <span>:</span> $ {{ Math.floor(price) }}
        </li>
        <li class="tracking-tight">
          <span class="font-semibold">Total price</span>
          <span class="text-red-500 font-semibold text-xl">:</span> $ {{ Math.floor(price) }}
          <span><XMarkIcon class="w-4 h-4 inline" /></span> <span>{{Math.floor(quantity)}}</span>
          <span>=</span> {{ Math.floor(price*quantity) }}
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  setup(props) {
    const {id,category,title,price,quantity,image} = props.purchedItemsInfo;
    return {
      getItemDetails,
      id,category,title,price,quantity,image
    };
  },
};
</script>
